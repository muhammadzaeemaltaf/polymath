...
const Search = ({ searchParams }: { searchParams: { query: string } }) => {
  ...
  useEffect(() => {
    ...
    if (searchParams.query) {
      setSearchResults({
        blogs: searchBlogs(searchParams.query).slice(0, 3),
        users: searchUsers(searchParams.query).slice(0, 3),
        categories: searchCategories(searchParams.query).slice(0, 3),
      });
      setSearchTerm("");
    }
  }, [searchParams.query]);
  ...
  const filteredBlogs = blogs.filter(
    (blog) =>
      blog.title.toLowerCase().includes(searchParams.query.toLowerCase()) ||
        ...
        <div className="flex flex-col-reverse lg:flex-row gap-4 lg:px-5 pb-4">
          <div className="lg:w-[70%] lg:border-r md:px-10">
            <h1 className="uppercase font-bold text-2xl text-center lg:text-start md:text-3xl lg:text-4xl pb-5 pt-10 lg:pr-5 ">
              Showing {start}-{end} of {filteredBlogs.length} results for{" "}
              {searchParams.query}
            </h1>
            <div className="">
